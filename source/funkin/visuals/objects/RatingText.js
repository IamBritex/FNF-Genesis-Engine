...
export class RatingText {
    ...
    init() {
        ...
        // Detectar downScroll desde localStorage
        const downScroll = localStorage.getItem('GAMEPLAY.NOTE SETTINGS.DOWNSCROLL') === 'true';
        let yPosition;
        if (downScroll) {
            // Arriba, separado 10px por encima de la healthbar (que suele estar en 25~30)
            yPosition = 10; // Puedes ajustar este valor si la healthbar cambia de tamaño
        } else {
            // Abajo por defecto
            yPosition = this.scene.scale.height - 30;
        }
        ...
        this.container.setDepth(101); // Mayor que la healthbar (que suele estar en 100)
