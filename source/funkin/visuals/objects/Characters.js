export class Characters {
  constructor(scene) {
    ...
    this.gfDanceDirection = 'right'; // Añadir esta nueva propiedad
  }
        ...
        if (animationFrames.length > 0) {
          ...
          if (!this.scene.anims.exists(animKey)) {
            this.scene.anims.create({
              ...
              frameRate: frameRate,
              repeat: 0 // Las animaciones de baile no se repiten
            });
          }
        }
        resolve();
  ...
  playGFIdleAnimation(characterId) {
    ...
    if (character.currentAnimation === "cheer" || character.currentAnimation === "sad") {
        return;
    }

    // Usar la dirección actual para determinar la animación
    const newAnimation = `dance${this.gfDanceDirection.charAt(0).toUpperCase() + this.gfDanceDirection.slice(1)}`;
    const animKey = `${character.textureKey}_${newAnimation}`;
    
    if (this.scene.anims.exists(animKey)) {
        character.sprite.stop();
        character.sprite.play(animKey);
        character.currentAnimation = newAnimation;
        this.applyOffsets(characterId, newAnimation);
  ...
  playAnimation(characterId, animName, force = false) {
    ...
    // Actualizar el beat actual para la animación
    character.lastAnimationBeat = this.lastBeat;

    const animation = character.data.animations.find(a => a.anim === animName);
    if (!animation) {
        console.warn(`Animation ${animName} not found for character ${characterId}`);
        if (animName !== "idle" && character.isGF) {
            // Si es GF y la animación no existe, volver al baile
            this.gfDanceDirection = 'left';
            this.playGFIdleAnimation(characterId);
        }
        return;
    }
  ...
  handleGFComboReactions(newCombo) {
    ...
    // Verificar si debemos cambiar la animación
    if (newCombo >= 50 && this.lastCombo < 50) {
        this.playAnimation(this.currentGF, "cheer", true);
        gf.lastAnimationBeat = this.lastBeat;
    } else if (newCombo === 0 && this.lastCombo >= 50) {
        this.playAnimation(this.currentGF, "sad", true);
        gf.lastAnimationBeat = this.lastBeat;
    }
  ...
  subscribeToNoteEvents(characterId) {
    ...
    this.notesController.events.on('strumlineStateChange', (data) => {
      ...
      // Si es GF, no interrumpir sus animaciones por las notas
  ...
  onBeat(beat) {
    ...
    for (const [characterId, character] of this.loadedCharacters) {
        ...
        if (character.isGF) {
            // GF alterna entre danceLeft y danceRight en cada beat
            if (character.currentAnimation === "cheer" || character.currentAnimation === "sad") {
                const lastAnimBeat = character.lastAnimationBeat ?? beat;
                if ((beat - lastAnimBeat) >= 2) {
                    this.gfDanceDirection = 'left';
                    character.currentAnimation = "danceLeft";
                    this.playGFIdleAnimation(characterId);
                }
                continue;
            }

            // Alternar dirección del baile en cada beat
            if (beat % 2 === 0) {
        ...
        // Resto del código para personajes normales...
        const directions = character.isPlayer ? this.heldDirections : this.enemyHeldDirections;
        const hasActiveNotes = directions.size > 0;
        
        const notesController = this.notesController;
        const isDestroyingPieces = character.isPlayer ? 
            notesController.activeHoldNotes.some(note => note && note.isBeingHeld) :
            notesController.activeEnemyHoldNotes.some(note => note && note.isBeingHeld);
        
        if (!hasActiveNotes && !isDestroyingPieces && character.currentAnimation !== "idle") {
            const lastAnimBeat = character.lastAnimationBeat ?? beat;
            if ((beat - lastAnimBeat) >= 2) {
                this.playAnimation(characterId, "idle", true);
                character.lastIdleBeat = beat;
            }
        } else if (!hasActiveNotes && !isDestroyingPieces && character.currentAnimation === "idle" && !character.sprite.anims.isPlaying) {
            this.playAnimation(characterId, "idle", true);
            character.lastIdleBeat = beat;
        }
    }
...
checkActiveNotes(characterId) {
    // Obtener el mapa de direcciones según si es jugador o enemigo
    const character = this.loadedCharacters.get(characterId);
    if (!character) return false;

    const directions = character.isPlayer ? this.heldDirections : this.enemyHeldDirections;
    
    // Si hay alguna dirección activa, significa que está presionando una nota
