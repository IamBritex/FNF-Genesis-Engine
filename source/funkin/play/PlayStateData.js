/**
 * PlayStateData.js
 * * Maneja la inicialización y gestión de datos para el PlayState.
 */
export class PlayStateData {
  /**
   * Inicializa los datos de la escena, buscándolos directamente o en el registry.
   * Esta lógica se extrajo de PlayState.init() para modularizar.
   * * @param {Phaser.Scene} scene - La instancia de la escena PlayState.
  ...
  static init(scene, data) {
    ...
    if (!playData || Object.keys(playData).length === 0) {
      ...
      playData = scene.registry.get("playStateData");

      if (playData) {
        // La limpieza del registry se hace aquí, al momento de leer.
        scene.registry.remove("playStateData");
      } else {
        // Si el registry tampoco tiene datos, registrar advertencia y usar {} por defecto
        console.warn("PlayState no pudo encontrar datos de inicialización!");
        playData = {};
      }
    }
    ...
    console.log("PlayStateData.init() usando datos:", initData);
  ...
  static shutdown(scene) {
    // En este diseño actual, PlayStateData no retiene ningún dato estático
    ...
    console.log("PlayStateData.shutdown() llamado. (No se requiere limpieza activa).");
  }
}