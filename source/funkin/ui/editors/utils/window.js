 ...
 * [MODIFICADO - GESTIÓN DE POPUPS Y CIERRE AUTOMÁTICO]
 */
export class ModularWindow {
    
    // Registro estático de popups abiertos
    static openPopups = [];
    ...
    static initGlobalListeners() {
        ...
        // Cerrar todos los popups hijos cuando la ventana principal se cierra o recarga
    ...
    constructor(scene, config) {
        ...
        // Referencia para restaurar DOM al cerrar popup
    ...
    createWindow() {
        ...
        // Cálculo manual para centrar el overlay de 1280x720
        const overlayW = 1280;
        const overlayH = 720;
        const overlayX = (sceneWidth - overlayW) / 2;
        const overlayY = (sceneHeight - overlayH) / 2;

        // Estilos inline para el overlay fijo y posicionado debajo de la ventana
        ...
        // Estilo para asegurar que la ventana quede sobre el overlay
        const windowZIndex = this.config.overlay ? 'z-index: 9999;' : '';

        // Forzar tamaño del contenedor al tamaño de la escena para evitar colapso (0x0)
        const containerStyle = `width: ${sceneWidth}px; height: ${sceneHeight}px;`;
                ...
                <div class="modular-window" style="width: ${this.config.width}px; ${heightStyle} top: ${initialTop}px; left: ${initialLeft}px; ${windowZIndex}">
                    ...
                    <div class="window-title-bar" style="cursor: ${this.config.move ? 'grab' : 'default'};">
                        ...
                        <div class="window-buttons">
                            ${this.config.popup ? `<button class="window-btn" data-action="popup" title="Abrir en nueva ventana"><img src="public/images/ui/editors/open_in_new.svg" alt="Pop"></button>` : ''}
                            ${this.config.minimize ? `<button class="window-btn" data-action="minimize"><img src="public/images/ui/editors/minimize.png" alt="_"></button>` : ''}
                            ${this.config.maximize ? `<button class="window-btn" data-action="maximize"><img src="public/images/ui/editors/maximize.png" alt="[]"></button>` : ''}
                            ${this.config.close ? `<button class="window-btn close" data-action="close"><img src="public/images/ui/editors/close.png" alt="X"></button>` : ''}
                        </div>
                    </div>
    ...
    openInPopup() {
        ...
        // Registrar popup para cierre automático
        ...
        this.popupWindow.addEventListener('beforeunload', () => {
            // Eliminar del registro al cerrar manualmente
    ...
    destroy() {
        ...
        if (this.domElement) {
            try { this.scene.sound.play('editorClose'); } catch (e) {}
    ...
    static getWindowCSS() {
        ...
        return `
            ...
            /* Overlay base eliminado aquí para usar estilos inline dinámicos en createWindow */
            .modular-window {
