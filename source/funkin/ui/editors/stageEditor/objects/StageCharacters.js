...
export class StageCharacters {
    ...
    updateCharacterData(charKey, newCharName, newIconName, onLoadedCallback) {
        ...
        if (oldCharName === newCharName) { if (onLoadedCallback) onLoadedCallback(); return; }
    ...
    onSwapJSONLoaded(key, type, data) {
        ...
        this.characterHandler.characterElements.preloadAtlases(
            ...
            new Map(this.characterHandler.loadedCharacterJSONs).set(charName, data)
        );
    ...
    cleanupCharacterAssets(charName) {
        ...
        if (this.scene.cache.json.exists(jsonKey)) this.scene.cache.json.remove(jsonKey);
    ...
    refreshCharacterSpritesFromData() {
        ...
        const getSafeBottomCenter = (el) => {
            ...
            if (this.scene.cameraEditor) {
                ...
                if (frames.length > 0) el.setFrame(frames[0]);
            }
